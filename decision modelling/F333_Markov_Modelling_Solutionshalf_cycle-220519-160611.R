...
# and we should check each row adds to 1
rowSums(trace.AZT.hc)

# if we want to use the half-cycle correction, we can make the trace.AZT equal to our half cycle corrected version of the trace.
# then all the remaining calculations below will be the same (we don't need to change the name for every calculation below). 
trace.AZT <- trace.AZT.hc
...
trace.comb.hc <- (trace.comb.seed[1:cycles,] + trace.comb[1:cycles,]) / 2 

# Take a look, and check the rows still add to 1
head(trace.comb.hc)
rowSums(trace.comb.hc)

# If we want to use the half-cycle correction, we make trace.comb equal to our half cycle corrected version.
trace.comb <- trace.comb.hc
