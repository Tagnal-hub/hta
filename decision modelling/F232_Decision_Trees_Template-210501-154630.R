...
testing.cases <-  test.probs.vec * test.cases.vec    ## expected value of cases for each pathways (hint: use vector multiplication)
testing.cases


## For the total costs and cases of the whole intervention arm (i.e. all 5 pathways)
# take the sum of the vectors 
testing.costs.total <-  sum(testing.costs)     ## total costs (hint: use sum())
...
notest.path.1 <- p.hiv * p.trans.control   ## pathway probability for no test, HIV positive and transmission
notest.cost.1 <-  0 ## costs associated with this pathway
notest.cases.1 <- 1    ## whether vertical transmission accurs or not in this pathway
...
incr.costs <- -(notesting.costs.total - testing.costs.total)
