...
prevalence.vector <- seq(from = 0, to = 1, by = 0.01)
...
plot(no.test.nmb$prevalence,no.test.nmb$nmb.treat, type="l", col = "blue", ylim = c(0, 30000)) ## TREAT ALL
lines(no.test.nmb$prevalence,no.test.nmb$nmb.notreat, col = "red") ## TREAT NONE
...
est.nmb.perfect.test <- function(prev, parameters = parameter.values, lam = lambda){
  ...
  cost.sick <- parameters[1,2] 
  cost.healthy <-  parameters[4,2]
  
  qaly.sick <-   parameters[1,3]
  qaly.healthy <-   parameters[4,3]
  
  cost <- prev * cost.sick + (1-prev) * cost.healthy   ## Estimate the costs for the sick and healthy here
  qaly <- prev * qaly.sick + (1-prev) * qaly.healthy   ## Estimate the QALYs for the sick and healthy here
  
  nmb.perfect <-  (qaly * lam) - cost   # Calculate the NMB of the perfect test
...
est.nmb.perfect.test(0.3) ## 30% prevalence

est.nmb.perfect.test(prevalence.vector) ## using the prevalence vector

# Now save the results using the prevalence values vector 
perfect.test.nmb <- est.nmb.perfect.test(prevalence.vector)
...
plot(no.test.nmb$prevalence,no.test.nmb$nmb.treat, type="l", col = "red", ylim = c(0, 30000)) ## TREAT ALL
lines(no.test.nmb$prevalence,no.test.nmb$nmb.notreat, col = "dark green") ## TREAT NONE 
lines(perfect.test.nmb$prevalence, perfect.test.nmb$nmb, col = "blue") ## an additional line is needed in this case for the prefect test scenario
...
evpdi.table <- data.frame(
  prevalence = perfect.test.nmb$prevalence,
  evpdi = evpdi
...
diagnostic.threshold <- seq(-4, 4, 0.25)
...
TPR <- 1 - pnorm(diagnostic.threshold, mean = test.char[1], sd = test.char[2]) ## use the diagnostic.threshold and test.char data (sensitivity)
FPR <- 1 - pnorm(diagnostic.threshold, mean = test.char[3], sd = test.char[4]) # 1 - specificity
...
false.pos <-  (1-prevalence)* FPR     ## false positive
true.neg <- (1- prevalence) * (1-FPR)    ## true negative
...
evci <- est.evci(prevalence.vector)
