...
#  Foundation Course Exercise 3: TEMPLATE FILE
...
A.sum <- sum(alpha.A2A, alpha.A2B, alpha.A2C, alpha.A2D)  ## total counts of transitions out of A
B.sum <- sum(alpha.B2B, alpha.B2C, alpha.B2D) ## total counts of transitions out of B
C.sum <- sum(alpha.C2C, alpha.C2D)## total counts of transitions out of C  
...
c.LAM <- 2068.5  ## Lamivudine drug cost 
...
dr.c <- 3.5/100 ## Annual discount rate - costs (%)
dr.o <- 3.5/100  ## Annual discount rate - benefits (%) 
...
B.SympHIV.AZT <- c(0, tp.B2B, tp.B2C, tp.B2D)    ## all of the transitions out of B to (A,B,C,D); remember if a transition can't occur that should be set to 0 e.g. c(0, tp.B2B, ...) and so on
C.AIDS.AZT <- c(0, 0, tp.C2C, tp.C2D)       ## all of the transitions out of C to (A,B,C,D)
D.Death <-   c(0, 0, 0, 1)        ## transitions from death to (A,B,C,D) - set 3 values to 0, and the remaining to 1...
...
# I've not entirely understood the bit about discounting: The issue was that the factor was wrong.
disc.ly.AZT <-  discount.factor.o %*% ly.AZT
##multiply the discount factor matrix and the outcome matrix of the AZT arm
disc.ly.AZT
...
A.AsympHIV.comb <- c((1-tp.A2B*RR - tp.A2C*RR - tp.A2D*RR), tp.A2B*RR, tp.A2C*RR, tp.A2D*RR)   ## vector showing transitions out of State A under combination therapy c( , , ,)
B.SympHIV.comb <- c(0, c(1-tp.B2C*RR - tp.B2D*RR), tp.B2C*RR, tp.B2D*RR)    ## vector showing transitions out of State B under combination therapy
C.AIDS.comb <- c(0, 0, c(1-tp.C2D*RR), tp.C2D*RR)
...
trace.comb <- matrix(data = NA, nrow = cycles, ncol = n.states) ## the length of the matrix is equivalent to the number of cycles
...
ly.comb <- trace.comb %*% LY  ## the new trace * the LY reward vector
ly.comb

undisc.ly.comb <- colSums(ly.comb) ## fill in the blank
undisc.ly.comb

disc.ly.comb <- discount.factor.o %*% ly.comb    ## fill in the multiplier that's been left blank
disc.ly.comb

#### cost 
cost.comb <- trace.comb %*% c.dmc + trace.comb %*% c.ccc + trace.comb %*% c.azt   ## define cost.comb with the new trace matrix and the previously used costs (such as c.dmc)

## we need to replace rows 1 and 2 to add in the cost of lamivudine for years 1 and 2
cost.comb[1,1] <- cost.comb[1,1] + (trace.comb %*% c.lam)[1,1]     ## reminder to keep the original costs + the additional costs
cost.comb[2,1] <- cost.comb[2,1] + (trace.comb %*% c.lam)[2,1]      ## reminder to keep the original costs + the additional costs
cost.comb
...
output <- c(inc.cost = disc.cost.comb - disc.cost.AZT,
            ...
            icer = NA)
output["icer"] <-  (disc.cost.comb - disc.cost.AZT)/(disc.ly.comb - disc.ly.AZT)
output
